...
class TestComputeLengthsAngles(chex.TestCase):
    ...
    @parameterized.named_parameters(
        ...
        ("monoclinic",
         ...
         [3.0, 4.0, 5.099019513592784], [90.0, 90.0, 78.69006752597979]),
    )
    def test_compute_lengths_angles_known_cells(
        ...
        var_compute_lengths_angles = self.variant(rh.ucell.compute_lengths_angles)
        lengths, angles = var_compute_lengths_angles(vectors_array)
        
        chex.assert_trees_all_close(lengths, jnp.array(expected_lengths), atol=1e-5)
        chex.assert_trees_all_close(angles, jnp.array(expected_angles), atol=1e-5)
...
"""
    ...
    def test_parse_cif_and_scrape_different_axes(
        ...
        chex.assert_trees_all_equal(
            filtered_crystal.cell_lengths,
            jnp.array([5.0, 5.0, 10.0])
        )
        chex.assert_trees_all_equal(
            filtered_crystal.cell_angles,
