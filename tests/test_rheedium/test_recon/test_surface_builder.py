...
and adsorbate layer addition.  Slabs are built once per class via
setUpClass to avoid redundant JIT compilations.
"""
...
def _make_mgo_crystal(a: float = 4.211) -> CrystalStructure:
    ...
    z_nums: np.ndarray = np.array([12.0, 12.0, 12.0, 12.0, 8.0, 8.0, 8.0, 8.0])
...
class TestCreateSurfaceSlab(chex.TestCase, parameterized.TestCase):
    ...
    def setUpClass(cls) -> None:
        """Build slabs once for the entire class."""
        super().setUpClass()
        cls.crystal = _make_cubic_crystal()
        cls.mgo_crystal = _make_mgo_crystal()
        cls.slab_001 = create_surface_slab(
            bulk_crystal=cls.crystal,
            surface_normal_miller=jnp.array([0, 0, 1], dtype=jnp.int32),
            slab_thickness_angstrom=10.0,
            vacuum_gap_angstrom=15.0,
        )
        cls.mgo_slab = create_surface_slab(
            bulk_crystal=cls.mgo_crystal,
            surface_normal_miller=jnp.array([0, 0, 1], dtype=jnp.int32),
    ...
    def test_cell_angles_valid(self) -> None:
        ...
        for i in range(3):
            angle: float = float(self.slab_001.cell_angles[i])
    ...
    def test_thicker_slab_has_more_atoms(self) -> None:
        ...
        assert self.slab_001.cart_positions.shape[0] >= (
    ...
    def test_frac_and_cart_shapes_match(self) -> None:
        ...
        chex.assert_shape(
            self.slab_001.frac_positions,
            self.slab_001.cart_positions.shape,
        )
...
class TestApplySurfaceReconstruction(chex.TestCase, parameterized.TestCase):
    ...
    @classmethod
    def setUpClass(cls) -> None:
        """Build slab once for the entire class."""
        super().setUpClass()
        crystal: CrystalStructure = _make_cubic_crystal(a=4.0)
        cls.slab = create_surface_slab(
            bulk_crystal=crystal,
            surface_normal_miller=jnp.array([0, 0, 1], dtype=jnp.int32),
            slab_thickness_angstrom=10.0,
            vacuum_gap_angstrom=15.0,
        )
        cls.recon_2x2 = apply_surface_reconstruction(
            slab=cls.slab,
    ...
    def test_reconstruction_returns_crystal(self) -> None:
        ...
        assert isinstance(self.recon_2x2, CrystalStructure)

    def test_2x2_expands_cell(self) -> None:
        """2x2 reconstruction should roughly double a and b."""
        original_a: float = float(self.slab.cell_lengths[0])
        original_b: float = float(self.slab.cell_lengths[1])
        new_a: float = float(self.recon_2x2.cell_lengths[0])
        new_b: float = float(self.recon_2x2.cell_lengths[1])
    ...
    def test_atom_count_scales_with_reconstruction(self) -> None:
        ...
        n_orig: int = self.slab.cart_positions.shape[0]
        n_recon: int = self.recon_2x2.cart_positions.shape[0]
        assert n_recon > n_orig
...
class TestAddAdsorbateLayer(chex.TestCase, parameterized.TestCase):
    ...
    def test_adsorbate_increases_atom_count(self) -> None:
        ...
        assert decorated.cart_positions.shape[0] == n_orig + 1
    ...
    def test_cell_parameters_unchanged(self) -> None:
        ...
        chex.assert_trees_all_close(
            ...
            self.slab.cell_lengths,
        )
        chex.assert_trees_all_close(
            decorated.cell_angles,
            self.slab.cell_angles,
        )
    ...
    def test_multiple_adsorbates(self) -> None:
        ...
        assert decorated.cart_positions.shape[0] == n_orig + 2
