...
@pytest.mark.xfail(reason="fixtures pending regeneration — FIX_PLAN Phase 12")
...
def test_benchmark_reference_suite_writes_json_summary() -> None:
    ...
    assert np.isfinite(_SUITE_PAYLOAD["mean_normalized_cross_correlation"])
    assert np.isfinite(_SUITE_PAYLOAD["mean_specular_offset_px"])
    assert np.isfinite(_SUITE_PAYLOAD["max_streak_fwhm_abs_error_px"])
