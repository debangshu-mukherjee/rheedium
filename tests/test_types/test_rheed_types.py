...
class TestRHEEDPattern(chex.TestCase):
    ...
    def test_create_rheed_pattern_valid(self) -> None:
        ...
        intensities = jax.random.uniform(self.rng, (n_reflections,), minval=0, maxval=1000)

        var_create_rheed_pattern = self.variant(create_rheed_pattern)
        pattern = var_create_rheed_pattern(g_indices, k_out, detector_points, intensities)
    ...
    def test_rheed_pattern_various_sizes(self, n_reflections: int, max_intensity: float) -> None:
        ...
        pattern = create_rheed_pattern(g_indices, k_out, detector_points, intensities)
        ...
        chex.assert_trees_all_equal(jnp.all(jnp.linalg.norm(pattern.k_out, axis=1) > 0), True)
    ...
    def test_rheed_pattern_validation_errors(self) -> None:
        ...
        with pytest.raises(ValueError, match=".*shape.*"):
            jax.jit(create_rheed_pattern)(g_indices, k_out, detector_points, intensities)
    ...
    def test_rheed_pattern_vmap(self) -> None:
        ...
        g_indices_batch = jnp.tile(jnp.arange(n_reflections, dtype=jnp.int32), (batch_size, 1))
...
class TestRHEEDImage(chex.TestCase):
    ...
    def test_create_rheed_image_valid(self) -> None:
        ...
        image = var_create_rheed_image(
            img_array, incoming_angle, calibration, electron_wavelength, detector_distance
        )
    ...
    def test_rheed_image_pytree(self) -> None:
        ...
        chex.assert_trees_all_equal(image.electron_wavelength, reconstructed.electron_wavelength)
        chex.assert_trees_all_equal(image.detector_distance, reconstructed.detector_distance)
    ...
    def test_rheed_image_various_params(
        ...
        chex.assert_scalar_positive(image.electron_wavelength)
        chex.assert_scalar_positive(image.detector_distance)
    ...
    def test_rheed_image_calibration_types(self) -> None:
        ...
        image_scalar = var_create_rheed_image(
            img_array, incoming_angle, scalar_calibration, electron_wavelength, detector_distance
        )
        chex.assert_trees_all_equal(
            jnp.isscalar(image_scalar.calibration) or image_scalar.calibration.ndim == 0, True
        )

        array_calibration = jnp.array([0.01, 0.015])
        image_array = var_create_rheed_image(
            img_array, incoming_angle, array_calibration, electron_wavelength, detector_distance
        )
    ...
    def test_rheed_image_jit_compilation(self) -> None:
        ...
        def create_and_process(
            ...
            image = create_rheed_image(img_array, angle, calibration, wavelength, distance)
        ...
        result = var_create_and_process(img_array, angle, calibration, wavelength, distance)
    ...
    def test_rheed_image_validation_errors(self) -> None:
        ...
        with pytest.raises(ValueError, match=".*wavelength.*"):
            jax.jit(create_rheed_image)(img_array, 2.0, 0.01, negative_wavelength, 1000.0)

        img_array = jnp.ones((64, 64))
        negative_calibration = -0.01
        with pytest.raises(ValueError, match=".*calibration.*"):
            jax.jit(create_rheed_image)(img_array, 2.0, negative_calibration, 0.037, 1000.0)
...
class TestRHEEDIntegration(chex.TestCase):
    ...
    def test_combined_rheed_structures(self) -> None:
        ...
        chex.assert_trees_all_close(combined["image"].img_array, reconstructed["image"].img_array)
