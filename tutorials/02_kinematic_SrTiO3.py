...
def _(mo):
    mo.md(
        r"""
    # Full Kinematic RHEED: SrTiO3 (001)

    This is the same high-level detector-image pipeline as the MgO tutorial,
    using a different material and beam geometry. The notebook stays compact by
    calling the new `rh.simul.simulate_detector_image` orchestrator.
    """
    )
    return
...
def _(mo, settings):
    ...
    Set `phi_deg = 45.0` if you want a [110]-style cubic azimuth instead.
    """
    )
    return
...
def _(detector_image, np, plt, rh, settings, sparse_pattern):
    ...
    plt.suptitle("SrTiO3(001) Kinematic RHEED", fontsize=16, y=1.02)
    plt.tight_layout()
    plt.show()
    return
