...
_ax.axhline(y=0.1, color="r", linestyle="--", alpha=0.5, label="σ = 0.1 Å⁻¹")
_ax.axhline(y=0.01, color="g", linestyle="--", alpha=0.5, label="σ = 0.01 Å⁻¹")
_ax.legend()
plt.tight_layout()
# Add reference lines
plt.show()
...
print("-" * 60)
for _name, _energy_spread_frac, _dtheta in beam_conditions:
    _sigma_shell = rh.simul.compute_shell_sigma(
        _k, _energy_spread_frac, _dtheta
    )
    print(
        f"{_name:<15} {_energy_spread_frac:<10.1e} "
        f"{_dtheta * 1000:<12.2f} {float(_sigma_shell):<15.4f}"
    )
print("\nFor comparison, rod width at L=100Å: σ_rod = 0.025 Å⁻¹")

# %%
_fig, _axes = plt.subplots(1, 3, figsize=(14, 4))
for _ax, (_name, _energy_spread_frac, _dtheta) in zip(_axes, beam_conditions):
    _overlap, _intensities = rh.simul.finite_domain_intensities(
        ewald=ewald,
        theta_deg=theta_deg,
        phi_deg=0.0,
        domain_extent_ang=domain_1,
        energy_spread_frac=_energy_spread_frac,
    ...
    _ax.set_title(
        f"{_name}\n(ΔE/E={_energy_spread_frac:.0e}, "
        f"Δθ={_dtheta * 1000:.1f}mrad)"
    )
    _n_active = int(jnp.sum(_overlap > 0.1))
    _ax.text(
        0.95,
